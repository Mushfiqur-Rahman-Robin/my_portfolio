...
- LLM cost tracking is managed via the `LLMCostTracking` model — a ledger-style table that records each chat or embedding API call with token counts, cost, and cumulative running totals (`total_chat_cost`, `total_embedding_cost`, `total_cost`, `total_chat_tokens`, `total_embedding_tokens`, `total_tokens`). Costs are stored as precise `Decimal` values with 4 decimal places (e.g., `0.2099`). Pricing is centralized in `api/pricing.py` and verified against current API provider pricing pages. The admin panel provides a read-only view.
- Chat costs are recorded per `ChatSession`; embedding costs (from RAG queries and content indexing) are recorded without session association. Token counting uses real API `usage_metadata`/`usage` when available, falling back to `tiktoken` estimation (`cl100k_base`). Row-level locking (`select_for_update()`) prevents race conditions on running totals.
